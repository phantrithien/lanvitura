        ...
        @foreach ($products as $product)
            <div class="bg-white rounded-lg shadow-md overflow-hidden group">
                ...
                <div class="p-4">
                    ...
                    <p class="text-gray-600 mt-2">{{ number_format($product->price) }} VNĐ</p>
                    <div class="mt-6">
                        <form action="{{ route('cart.add', $product->id) }}" method="POST">
                            <form action="{{ route('cart.add', $product->id) }}" method="POST" class="flex items-center gap-2 ajax-add-to-cart" data-action="{{ route('cart.add', $product->id) }}">
                            @csrf
                        </form>
                    </div>
                </div>
